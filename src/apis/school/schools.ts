...
import type { GetSchoolsParams, SchoolRequest, SchoolDetail, GetSchoolsResponse } from '@api/school/schools.d';
import { APIResponse } from "@apis/common";

// -------------------- 목록 조회 --------------------
export async function getSchools(params: GetSchoolsParams): Promise<GetSchoolsResponse> {
...
export async function getSchoolDetail(id: string): Promise<SchoolDetail> {
  const response = await axiosInstance.get<APIResponse<SchoolDetail>>(`/api/v1/admins/schools/${id}/details`);
  return response.data.data;
}
...
// -------------------- 수정 --------------------
export async function editSchool(id: number, data: SchoolRequest): Promise<void> {
  await axiosInstance.put(`/api/v1/admins/schools/${id}`, data);
}

export function useEditSchoolMutation() {
  return useMutation({
    mutationFn: ({ id, ...data }: { id: number } & SchoolRequest) => editSchool(id, data),
  });
}
