...
import { GetBannersParams, GetBannersResponse, GetBannerDetail, BannerBase, CreateBannerRequest, EditBannerRequest } from "@apis/banner/banners.d";
 ...
 */
  export async function getBanners(params: GetBannersParams): Promise<GetBannersResponse> {
    const response = await axiosInstance.get<APIResponse<GetBannersResponse>>("/api/v1/admins/banners/overviews", {
      params,
    });
 ...
 */
  export async function getBannerDetail(id: string): Promise<GetBannerDetail> {
    const response = await axiosInstance.get<APIResponse<GetBannerDetail>>(`/api/v1/admins/banners/${id}/details`);
    return response.data.data;
  }
  ...
  export const makeBannerFormData = ({
    ...
    const jsonBlob = new Blob(
      [JSON.stringify({ title, content, role })],
